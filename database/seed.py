...
for i in range(1, 11):
    ...
    db.execute("""INSERT INTO exercises (title, description, goal_weight, goal_set_amount, goal_rep_amount, user_id, category_id)
                  VALUES (?, ?, ?, ?, ?, ?, ?)""",
               [f"user1_exercise{i}", f"description{i}", random.randint(20, 100), random.randint(1, 5), random.randint(5, 20), 1, category_id])
...
for i in range(1, stat_count + 1):
    ...
    db.execute("""INSERT INTO stats (weight, set_amount, rep_amount, completed_at, user_id, exercise_id)
                  VALUES (?, ?, ?, datetime('now', ? || ' days'), ?, ?)""",
               [random.randint(20, 100), random.randint(1, 5), random.randint(5, 20), str(-random.randint(0, 365)), user_id, exercise_id])
...
for i in range(1, 1001):
    ...
    db.execute("""INSERT INTO stats (weight, set_amount, rep_amount, completed_at, user_id, exercise_id)
                  VALUES (?, ?, ?, datetime('now', ? || ' days'), ?, ?)""",
               [random.randint(20, 100), random.randint(1, 5), random.randint(5, 20), str(-random.randint(0, 365)), 1, exercise_id])
